...
if __name__ == '__main__':
    ...
    # Amazon: {Beauty, Books, Sports, Toys}
    # Tenrec: {QB_Article, Video: QB_Video, QK_Article, QK_Video}
    prefix = '/home/jyh/TriMLP4Rec/Data/'
    data_family = 'Tenrec'
    data_name = 'QK_Video'
    raw_data_path = prefix + data_family + '/' + data_name + '/' + data_name + '.inter'
    clean_data_path = prefix + data_family + '/' + data_name + '/' + data_name + '.data'
    data_statistic_path = prefix + data_family + '/' + data_name + '/' + data_name + '.txt'

    if os.path.exists(clean_data_path):
        dataset = un_serialize(clean_data_path)
    else:
        unpopular_loc_threshold = 10
        inactive_user_threshold = 20
        dataset = InterData(raw_data_path, unpopular_loc_threshold, inactive_user_threshold)
    count = 0
    length = []
    ...
    f = open(data_statistic_path, 'w')
    print("Data:", data_name, file=f)
